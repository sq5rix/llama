...
def compare_embeddings(pipe, soft_prompt_embeddings, real_prompt=PROMPT):
    ...
    input_ids = pipe.tokenizer(real_prompt, return_tensors="pt").input_ids.to(
        pipe.device
    )
    ...
    similarity = cosine_similarity(
        soft_prompt_embeddings, real_prompt_embeddings.mean(dim=1)
    )
    print(f"Similarity: {similarity}")


def fine_tune_soft_prompt(
    pipe,
    prompt,
    ...
    for step in range(num_steps):
        ...
        embeddings = torch.cat(
            [soft_prompt_embeddings.unsqueeze(0), prompt_embeddings], dim=1
        )
